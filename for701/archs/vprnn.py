...
class VPRNN(object):
    ...
    def __init__(self, input, n_in, n_hidden, n_out, activation=T.tanh,
        ...
        W_hx_init = np.asarray(np.random.uniform(size=(n_hidden, n_in),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_hx = theano.shared(value=W_hx_init, name="W_hx")

        W_hh_init = np.asarray(np.random.uniform(size=(n_hidden, n_hidden),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_hh = theano.shared(value=W_hh_init, name="W_hh")

        W_yh_init = np.asarray(np.random.uniform(size=(n_out, n_hidden),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_yh = theano.shared(value=W_yh_init, name="W_yh")
        '''
        self.ptr0 = theano.shared(value=0, name="ptr0")'''
        h0_init = np.zeros((n_hidden,), dtype=theano.config.floatX)
        self.h0 = theano.shared(value=h0_init, name="h0")
        '''
        stack0_init = np.zeros((100, n_hidden), dtype=theano.config.floatX)
        self.stack0 = theano.shared(value=stack0_init, name="stack0")'''
        self.params = [self.W_hx, self.W_hh, self.W_yh]
        '''
        self.n_hidden = n_hidden'''
        self.PUSH = np.zeros((n_in,), dtype=theano.config.floatX)
        self.POP = np.zeros((n_in,), dtype=theano.config.floatX)
        self.stack = []
        ...
        # activation function
        '''def step(x_t, h_tm1, stack_tm1, ptr_tm1):
            stack_t = T.switch(T.eq(x_t, self.PUSH),
                    T.set_subtensor(stack_tm1[ptr_tm1], h_tm1),
                    T.switch(T.eq(x_t, self.POP), 
                        T.set_subtensor(stack_tm1[ptr_tm1], 
                            T.zeros(self.n_hidden, theano.config.floatX)),
                        stack_tm1))
            ptr_t = T.switch(T.eq(x_t, self.PUSH), 
                    T.add(ptr_tm1, 1),
                    T.switch(T.eq(x_t, self.POP),
                        T.add(ptr_tm1, -1),
                        ptr_tm1))
            h_prime = T.switch(T.eq(x_t, self.PUSH),
                    h_tm1,
                    T.switch(T.eq(x_t, self.POP),
                        stack_tm1[ptr_tm1],
                        h_tm1))
            h_t = self.activation(T.dot(self.W_hx, x_t) + T.dot(self.W_hh, h_prime))
            y_t = T.dot(self.W_yh, h_t)
            return h_t, stack_t, ptr_t, y_t

        # the hidden state `h` for the entire sequence, and the output for the
        # entire sequence `y` (first dimension is always time)
        [self.h, self.stack, self.ptr, self.y_pred], _ = theano.scan(step,
                sequences=self.input,
                outputs_info=[self.h0, [self.stack0], [0], None])
'''
        def step(x_t, h_tm1):
            if T.eq(x_t, self.PUSH):
                self.stack.append(h_tm1)
                h_prime = h_tm1
            elif T.eq(x_t, self.POP):
                h_prime = self.stack.pop()
            else:
                h_prime = h_tm1
            h_t = self.activation(T.dot(self.W_hx, x_t) + T.dot(self.W_hh, h_prime))
            y_t = T.dot(self.W_yh, h_t)
...
class MetaVPRNN(BaseEstimator):
    ...
    def ready(self):
        ...
        if self.output_type == 'real':
            ...
            raise NotImplementedError
        # initial hidden state of the VPRNN
        self.h0 = T.vector()
        # learning rate
        self.lr = T.scalar()
        ...
        self.vprnn = VPRNN(input=self.x, n_in=self.n_in,
        ...
        if self.output_type == 'real':
            self.predict = theano.function(inputs=[self.x, ],
                                           outputs=self.vprnn.y_pred,
                                           mode=mode)
        elif self.output_type == 'binary':
            self.predict_proba = theano.function(inputs=[self.x, ],
                                outputs=self.vprnn.p_y_given_x, mode=mode)
            self.predict = theano.function(inputs=[self.x, ],
                                outputs=T.round(self.vprnn.p_y_given_x),
                                mode=mode)
        elif self.output_type == 'softmax':
            self.predict_proba = theano.function(inputs=[self.x, ],
                        outputs=self.vprnn.p_y_given_x, mode=mode)
            self.predict = theano.function(inputs=[self.x, ],
                                outputs=self.vprnn.y_out, mode=mode)
        else:
            raise NotImplementedError
    ...
    def __getstate__(self):
        ...
        params = None
        weights = [p.get_value() for p in self.vprnn.params]
    ...
    def _set_weights(self, weights):
        ...
        for param in self.vprnn.params:
    ...
    def fit(self, X_train, Y_train, X_test=None, Y_test=None,
        ...
        cost = self.vprnn.loss(self.y) \
            + self.L1_reg * self.vprnn.L1 \
            + self.L2_reg * self.vprnn.L2_sqr

        compute_train_error = theano.function(inputs=[index, ],
                                              outputs=self.vprnn.loss(self.y),
                                              givens={
        ...
        updates = {}
        for param, gparam in zip(self.vprnn.params, gparams):
            weight_update = self.vprnn.updates[param]
...
def test_real():
    """ Test VPRNN with real-valued outputs. """
    n_hidden = 10
    n_in = 5
    n_out = 3
    n_steps = 10
    ...
    model = MetaVPRNN(n_in=n_in, n_hidden=n_hidden, n_out=n_out,
...
def test_binary(multiple_out=False, n_epochs=250):
    """ Test VPRNN with binary outputs. """
    n_hidden = 10
    n_in = 5
    if multiple_out:
        n_out = 2
...
def test_softmax(n_epochs=250):
    """ Test VPRNN with softmax outputs. """
    n_hidden = 10
    n_in = 5
    n_steps = 10
    n_seq = 100
