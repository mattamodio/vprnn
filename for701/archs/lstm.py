...
class LSTM(object):
    ...
    def __init__(self, input, n_in, n_hidden, n_out, activation=T.tanh,
        ...
        # initialize a shit ton of weights
        W_ix_init = np.asarray(np.random.uniform(size=(n_hidden, n_in),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_ix = theano.shared(value=W_ix_init, name='W_ix')

        W_ih_init = np.asarray(np.random.uniform(size=(n_hidden, n_hidden),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_ih = theano.shared(value=W_ih_init, name='W_ih')

        W_ox_init = np.asarray(np.random.uniform(size=(n_hidden, n_in),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_ox = theano.shared(value=W_ox_init, name='W_ox')

        W_oh_init = np.asarray(np.random.uniform(size=(n_hidden, n_hidden),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_oh = theano.shared(value=W_oh_init, name='W_oh')

        W_fx_init = np.asarray(np.random.uniform(size=(n_hidden, n_in),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_fx = theano.shared(value=W_fx_init, name='W_fx')

        W_fh_init = np.asarray(np.random.uniform(size=(n_hidden, n_hidden),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_fh = theano.shared(value=W_fh_init, name='W_fh')

        W_gx_init = np.asarray(np.random.uniform(size=(n_hidden, n_in),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_gx = theano.shared(value=W_gx_init, name='W_gx')

        W_gh_init = np.asarray(np.random.uniform(size=(n_hidden, n_hidden),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_gh = theano.shared(value=W_gh_init, name='W_gh')

        W_yh_init = np.asarray(np.random.uniform(size=(n_out, n_hidden),
            low=-.01, high=.01), dtype=theano.config.floatX)
        self.W_yh = theano.shared(value=W_yh_init, name="W_yh")

        h0_init = np.zeros((n_hidden,), dtype=theano.config.floatX)
        self.h0 = theano.shared(value=h0_init, name='h0')

        c0_init = np.zeros((n_hidden,), dtype=theano.config.floatX)
        self.c0 = theano.shared(value=c0_init, name='c0')

        self.params = [self.W_ix, self.W_ih, self.W_ox, self.W_oh,
                self.W_fx, self.W_fh, self.W_gx, self.W_gh, 
                self.W_yh, self.h0, self.c0]
        ...
        def step(x_t, h_tm1, c_tm1):
            i = T.nnet.hard_sigmoid(T.dot(self.W_ix, x_t) + T.dot(self.W_ih, h_tm1))
            o = T.nnet.hard_sigmoid(T.dot(self.W_ox, x_t) + T.dot(self.W_oh, h_tm1))
            f = T.nnet.hard_sigmoid(T.dot(self.W_fx, x_t) + T.dot(self.W_fh, h_tm1))
            g = T.nnet.hard_sigmoid(T.dot(self.W_gx, x_t) + T.dot(self.W_gh, h_tm1))
            c_t = f * c_tm1 + i * g
            h_t = o * self.activation(c_t)
            y_t = T.dot(self.W_yh, h_t)
            return h_t, c_t, y_t
...
class MetaLSTM(BaseEstimator):
    ...
    def ready(self):
        ...
        self.lstm = LSTM(input=self.x, n_in=self.n_in,
        ...
        if self.output_type == 'real':
            self.predict = theano.function(inputs=[self.x, ],
                                           outputs=self.lstm.y_pred,
                                           mode=mode)
        elif self.output_type == 'binary':
            self.predict_proba = theano.function(inputs=[self.x, ],
                                outputs=self.lstm.p_y_given_x, mode=mode)
            self.predict = theano.function(inputs=[self.x, ],
                                outputs=T.round(self.lstm.p_y_given_x),
                                mode=mode)
        elif self.output_type == 'softmax':
            self.predict_proba = theano.function(inputs=[self.x, ],
                        outputs=self.lstm.p_y_given_x, mode=mode)
            self.predict = theano.function(inputs=[self.x, ],
                                outputs=self.lstm.y_out, mode=mode)
        else:
            raise NotImplementedError
    ...
    def __getstate__(self):
        ...
        weights = [p.get_value() for p in self.lstm.params]
    ...
    def fit(self, X_train, Y_train, X_test=None, Y_test=None,
        ...
        cost = self.lstm.loss(self.y) \
            + self.L1_reg * self.lstm.L1 \
            + self.L2_reg * self.lstm.L2_sqr

        compute_train_error = theano.function(inputs=[index, ],
                                              outputs=self.lstm.loss(self.y),
                                              givens={
        ...
        updates = {}
        for param, gparam in zip(self.lstm.params, gparams):
            weight_update = self.lstm.updates[param]
...
def test_real():
    ...
    model = MetaLSTM(n_in=n_in, n_hidden=n_hidden, n_out=n_out,
