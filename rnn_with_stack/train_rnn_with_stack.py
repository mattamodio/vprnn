...
def train_with_sgd(model, X_train, y_train, learning_rate=0.005, nepoch=1, evaluate_loss_after=100):
    ...
    for epoch in range(nepoch):
        ...
        if (epoch % evaluate_loss_after == 0):
            ...
            save_model_parameters_theano("saved_model_parameters/stack-rnn-theano-%d-%d-%s.npz" % (model.hidden_dim, model.word_dim, time), model)
...
model = StackRNNTheano(ALPHABET_LENGTH+1, hidden_dim=_HIDDEN_DIM)
