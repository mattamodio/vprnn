...
class LSTMTheano:
    ...
    def __init__(self, word_dim, hidden_dim=100, minibatch_size=1, bptt_truncate=4):
        ...
        W_x_i_2 = np.random.uniform(-np.sqrt(1./word_dim), np.sqrt(1./word_dim), (hidden_dim, word_dim))
        W_h_i_2 = np.random.uniform(-np.sqrt(1./word_dim), np.sqrt(1./word_dim), (hidden_dim, hidden_dim))
        W_x_o_2 = np.random.uniform(-np.sqrt(1./word_dim), np.sqrt(1./word_dim), (hidden_dim, word_dim))
        W_h_o_2 = np.random.uniform(-np.sqrt(1./word_dim), np.sqrt(1./word_dim), (hidden_dim, hidden_dim))
        W_x_f_2 = np.random.uniform(-np.sqrt(1./word_dim), np.sqrt(1./word_dim), (hidden_dim, word_dim))
        W_h_f_2 = np.random.uniform(-np.sqrt(1./word_dim), np.sqrt(1./word_dim), (hidden_dim, hidden_dim))
        W_x_g_2 = np.random.uniform(-np.sqrt(1./word_dim), np.sqrt(1./word_dim), (hidden_dim, word_dim))
    ...
    def __theano_build__(self):
        ...
        def forward_prop_step(x_t, h_t_prev, c_t_prev):
            ...
            i_2 = T.nnet.hard_sigmoid( W_x_i_2.dot(x_t) + W_h_i_2.dot(h_t) )
            o_2 = T.nnet.hard_sigmoid( W_x_o_2.dot(x_t) + W_h_o_2.dot(h_t) )
            f_2 = T.nnet.hard_sigmoid( W_x_f_2.dot(x_t) + W_h_f_2.dot(h_t) )
            g_2 = T.tanh( W_x_g_2.dot(x_t) + W_h_g_2.dot(h_t) )

            c_t_2 = f_2*c_t + i_2*g_2
        ...
        dW_x_i_2 = T.grad(o_error, W_x_i_2)
        dW_h_i_2 = T.grad(o_error, W_h_i_2)
        dW_x_o_2 = T.grad(o_error, W_x_o_2)
        dW_h_o_2 = T.grad(o_error, W_h_o_2)
        dW_x_f_2 = T.grad(o_error, W_x_f_2)
        dW_h_f_2 = T.grad(o_error, W_h_f_2)
        dW_x_g_2 = T.grad(o_error, W_x_g_2)
        dW_h_g_2 = T.grad(o_error, W_h_g_2)
        ...
        self.sgd_step = theano.function([x,y,learning_rate], [], 
                      updates=[(self.W_x_i, self.W_x_i - learning_rate * dW_x_i),
                              ...
                              (self.W_x_i_2, self.W_x_i_2 - learning_rate * dW_x_i_2),
                              (self.W_h_i_2, self.W_h_i_2 - learning_rate * dW_h_i_2),
                              (self.W_x_o_2, self.W_x_o_2 - learning_rate * dW_x_o_2),
                              (self.W_h_o_2, self.W_h_o_2 - learning_rate * dW_h_o_2),
                              (self.W_x_f_2, self.W_x_f_2 - learning_rate * dW_x_f_2),
                              (self.W_h_f_2, self.W_h_f_2 - learning_rate * dW_h_f_2),
                              (self.W_x_g_2, self.W_x_g_2 - learning_rate * dW_x_g_2),
                              (self.W_h_g_2, self.W_h_g_2 - learning_rate * dW_h_g_2)])
